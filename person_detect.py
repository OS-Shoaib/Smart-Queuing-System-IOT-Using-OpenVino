...
class PersonDetect:
    ...
    def predict(self, image):
        """
        Make asynchronous prediction from input
        :param image: input image
        :return: the output -prediction- and the image
        """
        input_image = self.preprocess_input(image)
        inference_request_handle = self.network.start_async(request_id=0, inputs=input_image)
        inference_status = inference_request_handle.wait()
        if inference_status == 0:
            outputs = inference_request_handle.outputs[self.output_name]
            detect = self.preprocess_outputs(outputs)
            return self.draw_outputs(detect, image)

    def draw_outputs(self, coords, image):
        """
        Draw boxes on output image
        :param coords: The coordination of predections
        :param image: input image
        :return: Boundary boxes on output image
        """
        w = image.shape[1]
        h = image.shape[0]
        detect = []
        for box in coords:
            ...
            detect.append([p1[0], p1[1], p2[0], p2[1]])
            image = cv2.rectangle(image, p1, p2, (0, 255, 0), 2)
        return detect, image

    def preprocess_outputs(self, outputs):
        """
        Preprocess the outputs
        :param outputs: the output from prediction
